...
def unDictionarize(dataDict):
    ...
    elif dataDict['dataType'] == 'Patient':
        from opentps.core.data._patient import Patient
        data = Patient()
        patientDataList = dataDict.get('patientData', [])
        # Remove patientData from dict before update to avoid overwriting with list of dicts
        if 'patientData' in dataDict:
             del dataDict['patientData']
             
        data.__dict__.update(dataDict)
        
        # Restore patientData
        for itemDict in patientDataList:
            item = unDictionarize(itemDict)
            if item:
                data.appendPatientData(item)

    elif dataDict['dataType'] == 'RTStruct':
        from opentps.core.data._rtStruct import RTStruct
        data = RTStruct()
        contours = dataDict.get('_contours', [])
        if '_contours' in dataDict:
            del dataDict['_contours']
            
        data.__dict__.update(dataDict)
        
        # Restore contours
        for contourDict in contours:
             contour = unDictionarize(contourDict)
             if contour:
                 data.appendContour(contour)

    elif dataDict['dataType'] == 'ROIContour':
